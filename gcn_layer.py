...
class Vanilla_GraphAttention(nn.Module):
    def __init__(self,
                 node_size,
                 rel_size,
                 triple_size,
                 ...
                 use_bias=False):
        super(Vanilla_GraphAttention, self).__init__()
        ...
        self.weight_marix = nn.ParameterList()
        ...
        for l in range(self.depth):
            attn_kernel = torch.nn.Parameter(data=torch.empty(self.node_dim*2, 1, dtype=torch.float32))
            torch.nn.init.xavier_uniform_(attn_kernel)
            self.attn_kernels.append(attn_kernel)

            weight_marix = torch.nn.Parameter(data=torch.empty(self.node_dim, self.node_dim, dtype=torch.float32))
            torch.nn.init.xavier_uniform_(weight_marix)
            self.weight_marix.append(weight_marix)
    ...
    def forward(self, inputs):
        ...
        for l in range(self.depth):
            ...
            # shape: [N_tri x dim]
            neighs = torch.mm(features[adj[1, :].long()],self.weight_marix[l])
            selfs =  torch.mm(features[adj[0, :].long()],self.weight_marix[l])

            concatfeature = torch.cat([selfs,neighs],dim=-1)

            # tri_rel = F.normalize(tri_rel, dim=1, p=2)
            # neighs = neighs - 2*torch.sum(neighs*tri_rel, dim=1, keepdim=True)*tri_rel

            att = torch.squeeze(torch.mm(concatfeature, attention_kernel), dim=-1)
...
class NR_GraphAttentionAL(nn.Module):
    def __init__(self,
                 node_size,
                 rel_size,
                 triple_size,
                 ...
                 attn_heads_reduction='concat',
                 use_bias=False):
        super(NR_GraphAttentionAL, self).__init__()
