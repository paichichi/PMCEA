...
class Encoder_Model_Al(nn.Module):
    def __init__(self, node_hidden, rel_hidden, triple_size, node_size, new_node_size, rel_size, device,
        ...
        super(Encoder_Model_Al, self).__init__()
    ...
    def gcn_forward(self):
        ...
        # ent_feature = self.enself.avg(self.ent_adj, self.ent_embedding.weight, self.node_size)
        ent_feature =self.ent_adj
        ...
        return out_ent_feature
        # return out_feature,img_out_feature

    def forward(self, train_paris:torch.Tensor):
        out_feature_join = self.gcn_forward()

        output_paire =  out_feature_join[train_paris[:, 0]]       
